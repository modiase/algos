...
def choose_activities_dp(activities: Collection[Activity]) -> ActivitiesByEnd:
    ...
    for i in range(N):
        ...
        c1, c2 = ( _insort([ activity for activity in c if activity.start >= current_activity.end or activity.end <= current_activity.start ], current_activity) for c in (c[i+1], c[i]))
...
if __name__ == '__main__':
    ...
    print(f"{seed=}")
    activities = [
        Activity(start:=rn.randint(0, 20), rn.randint(start+1, start + rn.randint(1, 10)))
        for _ in range(10)
    ]
    print(f"{activities=}")
