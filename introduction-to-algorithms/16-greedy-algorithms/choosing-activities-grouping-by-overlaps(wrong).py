...
def find_overlapping_activities(activities: Sequence[Activity]) -> Mapping[Activity, AbstractSet[Activity]]:
    if not activities:
        return {}
...
class PriorityQueue:
    def __init__(self, activities_with_overlaps: Mapping[Activity, AbstractSet[Activity]]):
        self._heap = [ (-len(value), (key, value)) for key, value in activities_with_overlaps.items() ]
        heapq.heapify(self._heap)
        self._index = { activity: i for i, (_, (activity, _)) in enumerate(self._heap) }

    def _swap(self, i: int, j: int) -> None:
        self._heap[i], self._heap[j] = self._heap[j], self._heap[i]
        self._index[self._heap[i][1][0]], self._index[self._heap[j][1][0]] = self._index[self._heap[j][1][0]], self._index[self._heap[i][1][0]]
    ...
    def remove_overlap(self, activity: Activity, overlapping_activity: Activity) -> None:
    ...
    def _sift_down(self, activity: Activity) -> None:
        while ((self.lchild(self._index[activity]) is not None and  self._heap[self._index[activity]][0] > self._heap[self.lchild(self._index[activity])][0]) 
        or (self.rchild(self._index[activity]) is not None and  self._heap[self._index[activity]][0] > self._heap[self.rchild(self._index[activity])][0])):
            if self._heap[self._index[activity]][0] > self._heap[self.lchild(self._index[activity])][0]:
...
def group_activities(activities: Collection[Activity]) -> Collection[Collection[Activity]]:
