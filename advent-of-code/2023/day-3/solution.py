...
def get_symbols(processed_lines: List[Tuple[int, List[Tuple[int, str]]]]) -> List[Tuple[int, int, str]]:
    result = []
...
def get_numbers(processed_lines: List[Tuple[int, List[Tuple[int, str]]]]) -> List[Tuple[int, int, str]]:
    result = []
    for row, ss in processed_lines:
        for col, s in ss:
...
def part_one(lines: List[str]):
    sum = 0
    processed_lines = [(lineno, parse_line(line))
...
def test_parse_line():
    ...
    for lineno, line in enumerate(lines, 1):
        ...
        assert (lineno, [int(t[2]) for t in get_numbers([processed_line])]) == (lineno, [int(s) for s in re.findall(
            r'\d+', line.strip() + '.')])
