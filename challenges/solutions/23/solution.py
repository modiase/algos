...
def valid_moves(start: TTile, matrix: TMatrix) -> List[TTile]:
    return [move for move in generate_moves(start=start)
            if tile_is_within_matrix_limits(tile=move, matrix=matrix)
            and tile_is_not_wall(tile=move, matrix=matrix)]


def compute_all_paths(start: TTile, end: TTile,
                      matrix: TMatrix) -> Optional[List[List[TTile]]]:
    def _compute_all_paths(start: TTile, end: TTile,
                           matrix: TMatrix, history: List[TTile]) -> Optional[List[List[TTile]]]:
        if start == end:
            return [[end]]

        next_starts = [next_start for next_start in valid_moves(start, matrix=matrix)
        ...
        for next_start in next_starts:
            computed_paths = _compute_all_paths(start=next_start,
                                                end=end, matrix=matrix,
                                                history=[*history, next_start])
    ...
    return _compute_all_paths(start=start, end=end,
                              matrix=matrix, history=[start])


def main(start: TTile, end: TTile,
         matrix: TMatrix) -> Optional[int]:
