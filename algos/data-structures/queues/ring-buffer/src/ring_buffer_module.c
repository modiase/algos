...
static PyObject* PyRingBuffer_new(PyTypeObject* type, PyObject* args, PyObject* kwds) {
...
static int PyRingBuffer_init(PyRingBuffer* self, PyObject* args, PyObject* kwds) {
    ...
    if (!self->buffer) {
        PyErr_SetString(PyExc_MemoryError, "Failed to create ring buffer");
        return -1;
    }
    return 0;
}
...
static PyObject* PyRingBuffer_enqueue(PyRingBuffer* self, PyObject* args) {
    ...
    if (!self->buffer) {
        PyErr_SetString(PyExc_RuntimeError, "Ring buffer not initialized");
        return NULL;
    }
...
static PyObject* PyRingBuffer_dequeue(PyRingBuffer* self, PyObject* Py_UNUSED(ignored)) {
    if (!self->buffer) {
        PyErr_SetString(PyExc_RuntimeError, "Ring buffer not initialized");
        return NULL;
    }
...
static PyTypeObject PyRingBufferType = {
    ...
    .tp_doc = "Dynamic ring buffer for Python objects with automatic resizing",
