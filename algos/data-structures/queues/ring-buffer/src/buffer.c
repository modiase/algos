...
struct ring_buffer *ring_buffer_create(size_t element_size)
{
	if (element_size == 0)
		return NULL;
	if (_DEFAULT_SIZE > SIZE_MAX / element_size)
		return NULL;
...
static void _ring_buffer_resize(struct ring_buffer *buffer)
{
	...
	for (size_t o = 0; o < buffer->size; o++) {
		memcpy((char *)new_data + o * buffer->element_size,
		       (char *)buffer->data +
		       ((buffer->head +
			 o) % buffer->capacity) * buffer->element_size,
		       buffer->element_size);
	}
	free(buffer->data);
...
static int _ring_buffer_enqueue_internal(struct ring_buffer *buffer,
					 const void *value)
{
	if (buffer->size == buffer->capacity) {
		return RING_BUFFER_FAILED;
	}
	memcpy((char *)buffer->data + buffer->tail * buffer->element_size,
...
static int _ring_buffer_dequeue_internal(struct ring_buffer *buffer,
					 void *value)
{
	if (buffer->size == 0) {
		return RING_BUFFER_FAILED;
	...
	memcpy(value,
	       (char *)buffer->data + buffer->head * buffer->element_size,
	       buffer->element_size);
